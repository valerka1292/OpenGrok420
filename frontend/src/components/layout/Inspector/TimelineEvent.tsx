...
export default function TimelineEvent({ event }: { event: Thought }) {
  ...
  return (
    <article
      className="border rounded-lg p-2 text-[12px] transition-all hover:translate-x-0.5"
      style={gradientStyle}
    >
      <div
        className="mb-1 font-mono inline-flex items-center gap-1"
            ...
            {(() => {
              ...
              return (
                <span
                  ...
                  style={{ color: getAgentColor(recipients[0]) }}
                >
                  {recipients[0] || event.to || "Unknown"}{" "}
                  <Lock size={10} className="text-amber-300" />
                </span>
              );
            })()}
          </div>
          <div className="text-text-primary/90 leading-relaxed font-mono mt-1.5">
            {event.content || "..."}
          </div>
        </div>
      ) : (
        <div className="text-text-primary/90 leading-relaxed font-mono mt-1.5">
          {event.content || event.query || event.tool || "..."}
        </div>
      )}
    </article>
  );
