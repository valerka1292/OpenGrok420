...
import { ChatSessionSummary } from '../store/useChat';
    ...
    return (
        <div className="flex flex-col h-full p-4">
            ...
            <div className="flex-1 overflow-y-auto -mx-2 px-2 space-y-4">
                <div>
                    <div className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-2 px-2">История</div>
                    <div className="space-y-1">
                        {visibleSessions.length === 0 && (
                            <div className="px-3 py-3 text-xs text-text-muted rounded-xl border border-border-subtle bg-bg-surface/40">
                        ...
                        {visibleSessions.map((item) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm text-left transition-colors group border ${isActive
                                        ? 'bg-accent-blue/15 border-accent-blue/35 text-text-primary'
                                ...
                                >
                                    ...
                                    <div className="min-w-0 flex-1">
                                        ...
                                        <div className="text-[10px] text-text-muted mt-0.5">{item.message_count} сообщений</div>
                                    </div>
                                </button>
                            );
                        })}
