...
const useChat = create<ChatState>((set, get) => ({
  ...
  queuePrompt: (prompt, autoSend = false) => set({ queuedPrompt: prompt, queuedPromptAutoSend: autoSend }),
  ...
  retryLastAssistant: () => {
    ...
    const previousUser = [...messages.slice(0, lastAssistantIndex)].reverse().find((m) => m.role === 'user');
  ...
  handleStreamEvent: (event) => {
    ...
    if (type === 'token') {
      set((state) => ({ currentResponse: state.currentResponse + String(event.content ?? '') }));
      return;
    }
    ...
    if (type === 'done') {
      ...
      if (currentResponse.trim()) {
        set((state) => ({
          isGenerating: false,
          messages: [...state.messages, { role: 'assistant', content: currentResponse, thoughts: [...currentThoughts], duration }],
          currentResponse: '',
          currentThoughts: [],
        ...
        set((state) => ({
          ...
          currentResponse: '',
          currentThoughts: [],
          currentStatus: lastError ? 'Ошибка генерации' : 'Нет данных от модели',
          startTime: 0,
          ...(lastError
            ? { messages: [...state.messages, { role: 'assistant', content: '', thoughts: [...currentThoughts], error: lastError }] }
            : {}),
        }));
      }
    }
